...
//nebarnix: added in order to track previous to next points for swivel angle calculation
//REAPLCE WITH CURRENT_POSITION WHEN POSSIBLE
//     float lastX=0;
//     float lastY=0;
     float lastAngle=0; //track swivel angle
...
//
//nebarnix: add in swivel moves
inline void line_to_destination(float fr_mm_s) {
  
  //make sure we're actually moving X and/or Y else don't compute swivel
  //if(abs(current_position[Y_AXIS] - destination[Y_AXIS]) > 0.00001 || abs(current_position[X_AXIS] - destination[X_AXIS]) > 0.00001)
  if((current_position[Y_AXIS] != destination[Y_AXIS]) || (current_position[X_AXIS] != destination[X_AXIS]) )
    { 
    //nebarnix: duplicate planner line here with JUST extruder steps 
    //How to find starting XYZ? I'm going to fudge this badly and just create a global variable to track XY points =O
    //But eventaually current_position should probably be used I guess (angle will be stored as mm not radians in this case)
    
    //maybe only do all this if you actually have a nonzero XY move, else skip.  
    float swivelTarget[XYZE];
    float newAngle = atan2(current_position[Y_AXIS]-destination[Y_AXIS],destination[X_AXIS]-current_position[X_AXIS])-M_PI/2.0; //this will be in 2*pi for 180 
    
    //constrain the new angle to less than pi from the old angle, so we always take the short path
    while((newAngle - lastAngle) > M_PI)
      newAngle -= 2.0*M_PI;
    
    while((newAngle - lastAngle) < -M_PI)
      newAngle += 2.0*M_PI;
    
    SERIAL_ECHOPAIR("From Y=", current_position[Y_AXIS]);
    SERIAL_ECHOPAIR(" to Y=", destination[Y_AXIS]);
    SERIAL_ECHOPAIR(": From X=", current_position[X_AXIS]);
    SERIAL_ECHOLNPAIR(" to X=", destination[X_AXIS]);
    
    SERIAL_ECHOPAIR("NewAngle=", newAngle);
    SERIAL_ECHOPAIR("rad OldAngle=", lastAngle);  
    SERIAL_ECHOLNPAIR("rad AngleMM=", (newAngle*10.0)/(2.0*M_PI));
    
    //SERIAL_ECHOLNPAIR(" y=", b);
    //convert radians to mm (0-2*pi to 0-10mm)
    
    destination[E_AXIS] = (newAngle*10.0)/(2.0*M_PI); //why does this only result in positive?
    //destination[E_AXIS] += -10;
    //JUST swivel first      
    swivelTarget[X_AXIS] = current_position[X_AXIS];
    swivelTarget[Y_AXIS] = current_position[Y_AXIS];
    swivelTarget[Z_AXIS] = current_position[Z_AXIS];
    swivelTarget[E_AXIS] = destination[E_AXIS];
    planner.buffer_line(swivelTarget[X_AXIS], swivelTarget[Y_AXIS], swivelTarget[Z_AXIS], swivelTarget[E_AXIS], 15.0, active_extruder);
...
static void do_homing_move(const AxisEnum axis, float distance, float fr_mm_s=0.0) {
  ...
  #else
    sync_plan_position(); //Set the planner/stepper positions directly from current_position with no kinematic translation
    
    //nebarnix: Disable vinyl feed Y axis home (just set to zero, no need to move)
    //We should use the opportunity to home the E0 axis, which is the swivel stepper.
    if (axis != Y_AXIS)  //only do this for X and Z 
      {
      current_position[axis] = distance;
      ...
      {
      //nebarnix: home extruder here using Ymin endstop but E axis. 
      current_position[E_AXIS] = 0;
      sync_plan_position(); //Set the planner/stepper positions directly from current_position with no kinematic translation
      
      current_position[E_AXIS] = distance; //a full rotation on the swivel. This shouldn't be a magic number...
      planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], HOMING_FEEDRATE_E, active_extruder);
      //nebarnix: above won't work unless we find a way to stop one stepper on a second limit switch but only for this move (the limit switch will be triggered constantly during blade swivel!)
      //if Y endstop is hit -- stop motion
      long int timeOutTest = millis();
      do{
         if(READ(Y_MIN_PIN) == Y_MIN_ENDSTOP_INVERTING)
            {
            stepper.endstop_triggered(E_AXIS); //force the move to stop 
            break; //break out of the loop
            }
         watchdog_reset();

         }while(planner.blocks_queued()); //only run this while there are steps remaining to be stepped. Maybe we missed home somehow
      
      
      //sync_plan_position(); //Set the planner/stepper positions directly from current_position with no kinematic translation
         
      //stepper.synchronize(); //wait for all buffered moves to finish
      //endstops.hit_on_purpose(); //need to call this to be able to continue moving n stuff
      
      //now reset it to be 0 position
      current_position[E_AXIS] = 0;
      sync_plan_position(); //Set the planner/stepper positions directly from current_position with no kinematic translation
            
      //move to the new home pos
      current_position[E_AXIS] = E_HOME_OFFSET;
      planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], HOMING_FEEDRATE_E*2.0, active_extruder);
      
      //now make THIS the new zero
      current_position[E_AXIS] = 0;
      sync_plan_position(); //Set the planner/stepper positions directly from current_position with no kinematic translation
      
      lastAngle = 0; //reset the tracked angle
      
      //set_home_offset(E_AXIS,E_HOME_OFFSET);
      //current_position[E_AXIS] = E_HOME_OFFSET;
      }           
...
static void homeaxis(const AxisEnum axis) {
  ...
  //nebarnix: added to keep the extruder axis from going forever as it thinks its homing the Y axis, which goes on forever
  if(axis == Y_AXIS)
     do_homing_move(axis, 1.5 * E_MAX_POS * axis_home_dir);
  else
 ...
 * Nebarnix edit : Home Z FIRST else we could ruin everything!
...
inline void gcode_G28() {
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    #if Z_HOME_DIR < 0  // If homing away from BED do Z first : reversed this for nebarnix hack
    ...
    #if Z_HOME_DIR > 0 //conditional swapped for nebarnix hack 
      if (home_all_axis || homeZ) {
        #if ENABLED(Z_SAFE_HOMING)
          home_z_safely();
        #else
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
   ...
   */
   //nebarnix: follow G0 and G1 here to add swivels
...
void prepare_move_to_destination() {
  ...
  void plan_arc(
    ...
    for (uint16_t i = 1; i < segments; i++) { // Iterate (segments-1) times
      ...
      //BEGIN SWIVEL CODE
      float swivelTarget[XYZE];
      float newAngle = atan2(current_position[Y_AXIS]-arc_target[Y_AXIS],arc_target[X_AXIS]-current_position[X_AXIS])-M_PI/2.0; //this will be in 2*pi for 180 
      ...
      SERIAL_ECHOPAIR("From Y=", current_position[Y_AXIS]);
      SERIAL_ECHOPAIR(" to Y=", arc_target[Y_AXIS]);
      SERIAL_ECHOPAIR(": From X=", current_position[X_AXIS]);
      SERIAL_ECHOLNPAIR(" to Y=", arc_target[X_AXIS]);
       
      SERIAL_ECHOPAIR("NewAngle=", newAngle);
      SERIAL_ECHOPAIR(" OldAngle=", lastAngle);  
      SERIAL_ECHOLNPAIR(" AngleMM=", (newAngle*10.0)/(2.0*M_PI));
       
      //SERIAL_ECHOPAIR("To X=", destination[X_AXIS]);
      //SERIAL_ECHOLNPAIR("To Y=", destination[Y_AXIS]);
      //SERIAL_ECHOLN('x');
      //convert radians to mm (0-2*pi to 0-10mm)
       
      destination[E_AXIS] = (newAngle*10.0)/(2.0*M_PI); 
      //JUST swivel first      
      swivelTarget[X_AXIS] = current_position[X_AXIS];
      swivelTarget[Y_AXIS] = current_position[Y_AXIS];
      swivelTarget[Z_AXIS] = current_position[Z_AXIS];
      swivelTarget[E_AXIS] = destination[E_AXIS];
      if(i<2) //first loop make sure we're facing the right way. The rest will 'follow' with small increments
         planner.buffer_line(swivelTarget[X_AXIS], swivelTarget[Y_AXIS], swivelTarget[Z_AXIS], swivelTarget[E_AXIS], 15.0, active_extruder);
      else
       planner.buffer_line(swivelTarget[X_AXIS], swivelTarget[Y_AXIS], swivelTarget[Z_AXIS], swivelTarget[E_AXIS], 5.0, active_extruder);
       
      lastAngle = newAngle;
      //END SWIVEL CODE
    
      //planner.buffer_line_kinematic(arc_target, fr_mm_s, active_extruder); //nebarnix: now do actual move, keep E axis the same to hold angle
      planner.buffer_line(arc_target[X_AXIS], arc_target[Y_AXIS], arc_target[Z_AXIS], swivelTarget[E_AXIS], fr_mm_s, active_extruder);
      current_position[X_AXIS] = arc_target[X_AXIS];
      current_position[Y_AXIS] = arc_target[Y_AXIS];
      current_position[Z_AXIS] = arc_target[Z_AXIS];
    }
    
    //BEGIN SWIVEL CODE 
    //For the last little catchup bit
    float swivelTarget[XYZE];
    float newAngle = atan2(current_position[Y_AXIS]-logical[Y_AXIS],logical[X_AXIS]-current_position[X_AXIS])-M_PI/2.0; //this will be in 2*pi for 180 
    ...
    SERIAL_ECHOPAIR("From Y=", current_position[Y_AXIS]);
    SERIAL_ECHOPAIR(" to Y=", logical[Y_AXIS]);
    SERIAL_ECHOPAIR(": From X=", current_position[X_AXIS]);
    SERIAL_ECHOLNPAIR(" to Y=", logical[X_AXIS]);
    ...
    swivelTarget[E_AXIS] = destination[E_AXIS];
    planner.buffer_line(swivelTarget[X_AXIS], swivelTarget[Y_AXIS], swivelTarget[Z_AXIS], swivelTarget[E_AXIS], 5.0, active_extruder);
    logical[E_AXIS] = destination[E_AXIS];
    lastAngle = newAngle;
    //END SWIVEL CODE
